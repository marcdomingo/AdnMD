    ...
    for i in range(len(combination_list)):
        for j in range(0,2*n):
            if j < n:
                ...
                module_list[i].append(centroid_list[i][j])  #j-1 amb 4
    #print(module_list)
    return module_list
def score(module_list):
    ...
    for i in range(len(module_list)):
        score_function[i].append(min(math.sqrt(pow(module_list[i][0]-x,2) + pow(module_list[i][1] - y,2) + pow(module_list[i][2]- z,2)),math.sqrt(pow(module_list[i][0]- y,2) + pow(module_list[i][1] - z,2) + pow(module_list[i][2]- x,2)), math.sqrt(pow(module_list[i][0]- z,2) + pow(module_list[i][1] - y,2) + pow(module_list[i][2]- x,2)),math.sqrt(pow(module_list[i][0]- z,2) + pow(module_list[i][1] - x,2) + pow(module_list[i][2]- y,2)), math.sqrt(pow(module_list[i][0]- y,2) + pow(module_list[i][1] - x,2) + pow(module_list[i][2]- z,2)),math.sqrt(pow(module_list[i][0]- x,2) + pow(module_list[i][1] - z,2) + pow(module_list[i][2]- y,2))))
        #Aquests numeros son les distancies entre els carbonis alfa i centroide del metal binding site que vull mimetitzar
        score_function[i].append(int(module_list[i][3]))
...
def mutate(sorted_score):
    ...
    for i in range(len(sorted_score)):
        ...
        os.system("/gpfs/projects/bsc72/Schrodinger_SMP2/utilities/structconvert -imae mutated_structures.mae -opdb merged_"+str(i)+"_"+ list_mutations[0]+"_"+list_mutations[1]+"_"+list_mutations[2]+".pdb")
        i = i + 1
        if i>1000:
            break
    ...
    #for filename in os.listdir('.'):
     #   if filename.startswith("propkamerged_"):
        ...
        for line in input:
            ...
            if (('ND1 HIS A '+str(sorted_sc[i][1])+' ') in line or ('ND1 HIS A  '+str(sorted_sc[i][1])+' ') in line or ('ND1 HIS A   '+str(sorted_sc[i][1])+' ') in line or  ('ND1 HIS A '+str(sorted_sc[i][2])+' ') in line or ('ND1 HIS A  '+str(sorted_sc[i][2])+' ') in line or ('ND1 HIS A   '+str(sorted_sc[i][2])+' ') in line or  ('ND1 HIS A '+str(sorted_sc[i][3])+' ') in line or ('ND1 HIS A  '+str(sorted_sc[i][3])+' ') in line or ('ND1 HIS A   '+str(sorted_sc[i][3])+' ') in line  )and 'ATOM' in line:
                nd1=line
            elif (('NE2 HIS A '+str(sorted_sc[i][1])+' ') in line or ('NE2 HIS A  '+str(sorted_sc[i][1])+' ') in line or ('NE2 HIS A   '+str(sorted_sc[i][1])+' ') in line or ('NE2 HIS A '+str(sorted_sc[i][2])+' ') in line or ('NE2 HIS A  '+str(sorted_sc[i][2])+' ') in line or ('NE2 HIS A   '+str(sorted_sc[i][2])+' ') in line or  ('NE2 HIS A '+str(sorted_sc[i][3])+' ') in line or ('NE2 HIS A  '+str(sorted_sc[i][3])+' ') in line or ('NE2 HIS A   '+str(sorted_sc[i][3])+' ') in line)and 'ATOM' in line:
                ne2=line
            elif (('HD1 HIS A '+str(sorted_sc[i][1])+' ') in line or ('HD1 HIS A  '+str(sorted_sc[i][1])+' ') in line or ('HD1 HIS A   '+str(sorted_sc[i][1])+' ') in line or  ('HD1 HIS A '+str(sorted_sc[i][2])+' ') in line or ('HD1 HIS A  '+str(sorted_sc[i][2])+' ') in line or ('HD1 HIS A   '+str(sorted_sc[i][2])+' ') in line or  ('HD1 HIS A '+str(sorted_sc[i][3])+' ') in line or ('HD1 HIS A  '+str(sorted_sc[i][3])+' ') in line  or ('HD1 HIS A   '+str(sorted_sc[i][3])+' ') in line) and 'ATOM' in line and ('HE2 HIS A') not in previous_line:
                output.write(ne2)
            elif (('HE2 HIS A '+str(sorted_sc[i][1])+' ') in line or ('HE2 HIS A  '+str(sorted_sc[i][1])+' ') in line or ('HE2 HIS A   '+str(sorted_sc[i][1])+' ') in line or  ('HE2 HIS A '+str(sorted_sc[i][2])+' ') in line or ('HE2 HIS A  '+str(sorted_sc[i][2])+' ') in line or ('HE2 HIS A   '+str(sorted_sc[i][2])+' ') in line or  ('HE2 HIS A '+str(sorted_sc[i][3])+' ') in line or ('HE2 HIS A  '+str(sorted_sc[i][3])+' ') in line or ('HE2 HIS A   '+str(sorted_sc[i][3])+' ') in line) and 'ATOM' in line and ('HD1 HIS A') not in previous_line:
                output.write(nd1)
            previous_line=line
...
def nitro_coordinates(input):
    ...
    #Split the list in list of list
    geometry2=[geometry[i*12:(i+1)*12] for i in range((len(geometry)+12-1)//12)]
    geometry3=list(geometry2)
    #print geometry2
    for j in range(len(geometry2)):
        geometry3[j]=[geometry2[j][i*4:(i+1)*4] for i in range(3)]
    #print geometry3
    return geometry3
...
def nitro_score(module_list):
    ...
    for i in range(len(module_list)):
        score_function[i].append(math.sqrt(pow(module_list[i][0]-2.0,2) + pow(module_list[i][1] - 2.0,2) + pow(module_list[i][2]- 2.0,2)))
        #Aquests numeros son les distancies entre els carbonis alfa i centroide del metal binding site que vull mimetitzar
        score_function[i].append(int(module_list[i][3]))
        score_function[i].append(int(module_list[i][4]))
        score_function[i].append(int(module_list[i][5]))
...
combination_list=combination(list_of_coordinates(input),3)
input.close()
centroid_list=centroid(combination_list,3)
module_list=module(combination_list,centroid_list,3)
output = open("final_score", "wt")
formated_function, sorted_score=score(module_list)
mutate(formated_function)
output.close()
propka()
#minimization()
output = open("nitrogen_filtred", "wt")
nitro_filter(sorted_score)
#input.close()
output.close()
...
nitro_module_list=nitro_module(nitro_list, centroid_list, 3)
